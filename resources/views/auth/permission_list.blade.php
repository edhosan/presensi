          ...
          <div class="span12">
            <div class="widget">
              ...
              <div class="widget-content">
                <table class="table table-striped table-bordered" id="permission-table" width="100%" >
                    <thead>
                        <tr>
                            <th></th>
                            <th>Id</th>
...
$(function() {
    var table = $('#permission-table').DataTable({
        ...
        buttons: [
            ...
            },
            {
                ...
                action: function ( e, dt, node, config ) {
                    ...
                    var newUrl = "{{ url('permission_edit') }}";
                  ...
                  toastr.info("Apakah anda yakin ingin menghapus data ini?<br/><button type='button' id='confirmYes' class='btn btn-danger'>Ya</button> <button type='button' id='confirmNo' class='btn'>Tidak</button>",'Konfirmasi?',
                  {
                      ...
                      onShown: function (toast) {
                          $("#confirmYes").click(function(){
                            ...
                            $.ajax({
                                url: '{{ url("api/permission_delete?api_token=") }}{{ Auth::user()->api_token }}',
                                type: 'post',
                                dataType: "json",
                                data: { data: arrData },
        ...
        processing: true,
        serverSide: true,
        ajax: '{{ url("api/permission_list?api_token=") }}{{ Auth::user()->api_token }}',
        columns: [
