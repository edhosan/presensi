...
<div class="main">
  <div class="main-inner">
      <div class="container">
          {!! Breadcrumbs::render('referensi.tpp') !!}
          <div class="row">
            <div class="span12">
              <div class="widget">
                <div class="widget-header">
                  <i class="icon-list"></i>
                  <h3>Daftar Standarisasi Biaya TPP</h3>
                </div>
...
$(function() {
    var table = $('#kategori-table').DataTable({
        ...
        buttons: [
            {
                ...
                action: function ( e, dt, node, config ) {
                  window.location.href = "{{ route('tpp.kategori.create') }}";
                }
            },
            {
                  ...
                  toastr.info("Apakah anda yakin ingin menghapus data ini?<br/><button type='button' id='confirmYes' class='btn btn-danger'>Ya</button> <button type='button' id='confirmNo' class='btn'>Tidak</button>",'Konfirmasi?',
                  {
                      ...
                      onShown: function (toast) {
                          $("#confirmYes").click(function(){
                            ...
                            $.ajax({
                                url: '{{ url("api/delete_kategori?api_token=") }}{{ Auth::user()->api_token }}',
                                type: 'post',
                                dataType: "json",
                                data: { data: arrData },
            ...
            {
              text: '<i class="icon-list"> Jenis Pengeluaran</i>',
              action: function ( e, dt, node, config ) {
                  var data = dt.row( { selected: true } ).data();
                  var newUrl = "{{ url('/tpp/jenis_pengeluaran') }}";
                  window.location.href = newUrl+"/"+data.id;
              },
              enabled: false
            },
        ...
        processing: true,
        serverSide: true,
        ajax: '{{ url("api/get_kategori?api_token=") }}{{ Auth::user()->api_token }}',
        columns: [
